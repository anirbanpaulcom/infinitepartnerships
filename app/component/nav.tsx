...
import {
  ...
  LinkedInLogoIcon,
  TwitterLogoIcon,
  ArrowLeftIcon, DotsHorizontalIcon
...
export function Navbar() {
  ...
  return (
    <motion.nav        
    style={{ height: useTransform(scrollY, [0, 50], ["100px", "-0px"]) , opacity: useTransform(scrollY, [0, 50], [1, 0])}}
    initial={false}  
...
function MobileNav({router} : {router: AppRouterInstance})  {
  ...
  useClickOutside(containerRef as React.RefObject<HTMLElement>, () => setMore(false));

  const menu = [
    { label: 'Home', icon: <Image src="/favicon.ico" alt="Favicon" width={24} height={24} className='rounded-lg' />, onClick: () => router.push('/') },
    { label: 'More', icon: <DotsHorizontalIcon className='size-5' />, onClick: () => setMore(true) }
  ];
  
  const more = [
    { label: 'Menu', icon: <ArrowLeftIcon className='size-5' />, onClick: () => setMore(false) },
    { label: 'LinkedIn', icon: <LinkedInLogoIcon className="size-5" />, onClick: () => {} },
    { label: 'GitHub', icon: <GitHubLogoIcon className="size-5" />, onClick: () => {} },
    { label: 'Email', icon: <BsEnvelopeFill className="size-5" />, onClick: () => {} },
    { label: 'Instagram', icon: <InstagramLogoIcon className="size-5" />, onClick: () => {} }
  ];

  const mobileIcons = (icons: Array<{label: string, icon: React.ReactNode, onClick: ()=> void}>) => (
...
function DesktopNav(){
  ...
  return (
    <div className={cn("flex justify-between items-center w-full h-full px-20 backdrop-blur-xl bg-opacity-50")}>
        <HoverDropdown 
          trigger={
          <Image
        ...
        <Link
          ...
          >
            <Logo/>
            <P3 className='my-4'>
            Be in the Know Possibility is endless, we can build your dreamed mobiles apps by your way.
            </P3>
            <div className="flex justify-between mt-2">
              <GitHubLogoIcon
                className="size-10 hover:bg-gray rounded-lg p-2"
              />
              <InstagramLogoIcon
                className="size-10 hover:bg-gray rounded-lg p-2"
              />
      ...
      <div className="flex space-x-4 items-center ">
        ...
        <button className="h-10 w-28 rounded-full bg-orange-700 hover:bg-white hover:text-black  opacity-80 text-white  font-extrabold  cursor-pointer p-2 my-4">Get Quote</button>
